...
public:
    ...
    void displayTaxiDetails(const pqxx::result::const_iterator& row) {
        ...
        std::unique_ptr<Taxi> taxi = make_unique<Taxi>(
                brand, model, color,
        ...
        std::cout << "Has Driver: " << (taxi->getHasDriver() ? "Yes" : "No") << std::endl;
        std::cout << "Has WiFi: " << (taxi->getHasWiFi() ? "Yes" : "No") << std::endl;
        std::cout << "Has Child Seat: " << (taxi->getHasChildSeat() ? "Yes" : "No") << std::endl;
